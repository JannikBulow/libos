...
typedef const os_char*  os_cstring; // c as in constant
...
LIBOS_EXPORT os_result __os_set_and_return_result(os_result result); // INTERNAL USE ONLY. Sets the last error in the current thread to `result` if `result` is less than 0, then returns the same result.
