...
// This means that positive values can be treated as warnings rather than errors.
enum os_error {
...
os_i64 os_last_platform_error(void);                // Returns the latest platform-dependent error code in the form of the largest possible integer.
os_string os_platform_error_getname(os_i64 error);  // Returns the platform-dependent name associated with a given error code. May return an empty string if there is no name. Return value must be released using `os_free_message`.
os_string os_platform_error_describe(os_i64 error); // Returns the platform-dependent description associated with a given error code. May return an empty string if there is no description. Return value must be released using `os_free_message`.

os_result os_last_error(void);                      // Returns the last error
os_string os_result_getname(os_result res);         // Returns the name associated with a given result (e.g. `-10` -> `"OS_ERROR_NO_MEMORY"`). Return value must be released using `os_free_message`.
os_string os_result_describe(os_result res);        // Returns the description associated with a given result (e.g. `-10` -> `"OS_ERROR_NO_MEMORY"`). Return value must be released using `os_free_message`.

void os_free_message(os_string message);            // The library may communicate in human-readable messages (mainly to describe errors) in string form, and these strings are copied into a unique buffer to avoid the user changing the description of an error. So the messages must be freed using this uniform function.
